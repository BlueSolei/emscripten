...
var LibrarySDL = {
  ...
  $SDL: {
    ...
    flushEventsToHandler: function() {
      ...
      while (SDL.pollEvent(SDL.eventHandlerTemp)) {
        {{{ makeDynCallBound('iii', 'SDL.eventHandler') }}}(SDL.eventHandlerContext, SDL.eventHandlerTemp);
      }
    },
    ...
    try {
      ...
      SDL.audio.queueNewAudioData = function SDL_queueNewAudioData() {
        ...
        for(var i = 0; i < SDL.audio.numSimultaneouslyQueuedBuffers; ++i) {
          ...
          {{{ makeDynCallBound('viii', 'SDL.audio.callback') }}}(SDL.audio.userdata, SDL.audio.buffer, SDL.audio.bufferSize);
  ...
  Mix_PlayChannel: function(channel, id, loops) {
    ...
    audio['onended'] = function SDL_audio_onended() { // TODO: cache these
      ...
      if (SDL.channelFinished)  {{{ makeDynCallBound('vi', 'SDL.channelFinished') }}}(channel);
    }
  ...
  Mix_HaltChannel: function(channel) {
    function halt(channel) {
      ...
      if (SDL.channelFinished) {
        {{{ makeDynCallBound('vi', 'SDL.channelFinished') }}}(channel);
      }
    }
    if (channel != -1) {
      halt(channel);
  ...
  Mix_HaltMusic: function() {
    ...
    if (SDL.hookMusicFinished) {
      {{{ makeDynCallBound('v', 'SDL.hookMusicFinished') }}}();
    }
    return 0;
  },
  ...
  SDL_AddTimer: function(interval, callback, param) {
    return window.setTimeout(function() {
      {{{ makeDynCallBound('iii', 'callback') }}}(interval, param);
    }, interval);
  },
